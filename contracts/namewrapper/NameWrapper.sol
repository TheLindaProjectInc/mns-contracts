...
import "../mrxregistrar/BaseRegistrar.sol";
    ...
    IERC721Receiver
{
    ...
    bytes32 private constant MRX_NODE =
    ...
    constructor(
        ...
        /* Burn CANNOT_REPLACE_SUBDOMAIN and CANNOT_UNWRAP fuses for ROOT_NODE and MRX_NODE */
    ...
    /**
     * @notice Wraps a .mrx domain, creating a new token and sending the original ERC721 token to this
     *         contract
     * @dev Can be called by the owner of the name in the .mrx registrar or an authorised caller on the
     *      registrar
     * @param label label as a string of the .mrx domain to wrap
    ...
    function wrapMRX2LD(
        ...
        require(
            ...
            "NameWrapper: Sender is not owner or authorised by the owner or authorised on the .mrx registrar"
        );
        ...
        _wrapMRX2LD(label, wrappedOwner, _fuses, resolver);
    }

    /**
     * @dev Registers a new .mrx second-level domain and wraps it.
     *      Only callable by authorised controllers.
     * @param label The label to register (Eg, 'foo' for 'foo.mrx').
     ...
     */
    function registerAndWrapMRX2LD(
        string calldata label,
        address wrappedOwner,
        uint256 duration,
        address resolver,
    ...
    /**
     * @dev Renews a .mrx second-level domain.
     *      Only callable by authorised controllers.
     * @param tokenId The hash of the label to register (eg, `keccak256('foo')`, for 'foo.mrx').
    ...
    /**
     * @notice Wraps a non .mrx domain, of any kind. Could be a DNSSEC name vitalik.xyz or a subdomain
    ...
    function wrap(
        ...
        require(
            parentNode != MRX_NODE,
            "NameWrapper: .mrx domains need to use wrapMRX2LD()"
        );
    ...
    /**
     * @notice Unwraps a .mrx domain. e.g. vitalik.mrx
     * @dev Can be called by the owner in the wrapper or an authorised caller in the wrapper
     * @param label label as a string of the .mrx domain to wrap e.g. vitalik.xyz would be 'vitalik'
     * @param newRegistrant sets the owner in the .mrx registrar to this address
     * @param newController sets the owner in the registry to this address
     */

    function unwrapMRX2LD(
        bytes32 label,
        address newRegistrant,
        address newController
    ) public override onlyTokenOwner(_makeNode(MRX_NODE, label)) {
        _unwrap(_makeNode(MRX_NODE, label), newController);
        registrar.transferFrom(address(this), newRegistrant, uint256(label));
    }

    /**
     * @notice Unwraps a non .mrx domain, of any kind. Could be a DNSSEC name vitalik.xyz or a subdomain
     * @dev Can be called by the owner in the wrapper or an authorised caller in the wrapper
    ...
    function unwrap(
        ...
        require(
            parentNode != MRX_NODE,
            "NameWrapper: .mrx names must be unwrapped with unwrapMRX2LD()"
        );
    ...
    function onERC721Received(
        ...
        //check if it's the mrx registrar ERC721
        require(
            msg.sender == address(registrar),
            "NameWrapper: Wrapper only supports .mrx ERC721 token transfers"
        );
        ...
        _wrapMRX2LD(label, owner, fuses, resolver);
    ...
    function _wrapMRX2LD(
        ...
        bytes32 node = _makeNode(MRX_NODE, labelhash);
        bytes memory name = _addLabel(label, "\x03mrx\x00");
    ...
    function _checkOwnership(
        ...
        if (parentNode == MRX_NODE) {
            // Special case .mrx: Check registrant or name isexpired
