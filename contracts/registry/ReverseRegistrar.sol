...
contract ReverseRegistrar is Ownable, Controllable {
     ...
     */
    constructor(MNS mnsAddr, NameResolver resolverAddr) {
        mns = mnsAddr;
    ...
    /**
     * @dev Transfers ownership of the reverse MNS record specified with the
     *      address provided
     * @param addr The reverse record to set
     * @param owner The address to set as the owner of the reverse record in MNS.
     * @param resolver The address of the resolver to set; 0 to leave unchanged.
    ...
    function setNameForAddr(
        ...
        defaultResolver.setName(_node, name);
        mns.setSubnodeOwner(ADDR_REVERSE_NODE, sha3HexAddress(addr), owner);
        return _node;
    }
     ...
     *      hexadecimal representation of an MetrixCoin address.
     * @param addr The address to hash
